...
### Story 4: 作为用户，我希望能够自定义记忆召回的参数
**场景**：
> 用户发现 AI 好友经常引用一些不太相关的旧事件，觉得记忆召回"过于敏感"。
> 
> 用户进入「设置」→「记忆设置」，将「相似度阈值」从 0.5 调整到 0.7，同时将「Event 召回数量」从 5 降低到 3。
> 
> 调整后，AI 好友的回复更加精准，只会在高度相关时才引用历史记忆。

**配置项说明**：
| 配置项 | 场景应用 |
| :--- | :--- |
| 启用记忆召回 | 用户想要"纯净"对话时，可关闭此功能 |
| 回忆搜索轮数 | 调低可加快响应速度，调高可找到更深层记忆 |
| Profile 召回数量 | 控制注入 AI 的用户画像条目数 |
| Event 召回数量 | 控制注入的历史事件条目数 |
| 相似度阈值 | 调高可减少"牵强"的记忆关联，调低可增加记忆覆盖面 |

**验收标准**：
- 用户可在设置界面中自由调整记忆召回参数。
- 参数修改后立即生效，无需重启应用。
- 参数有合理的范围限制和默认值。
- 提供参数说明，让用户理解每个配置项的作用。

### Story 5: 作为用户，我希望能够控制是否展示 AI 的思考过程
**场景**：
> 用户是一名技术人员，想要了解 AI 是如何利用记忆来生成回复的。
> 
> 用户进入「设置」→「聊天设置」，开启「显示工具调用」和「显示思维链」。
> 
> 之后的对话中，用户可以看到 AI 调用 `recall_memory` 工具的过程，以及回忆 Agent 的推理步骤。

**验收标准**：
- 默认情况下，工具调用和思维链内容对用户隐藏。
- 开启开关后，用户可以清晰地看到 AI 的"幕后工作"。
- 展示形式简洁易懂，不影响主对话流程。
